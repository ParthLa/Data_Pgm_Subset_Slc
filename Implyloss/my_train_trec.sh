...
# declare -a model_id=(1) # (5 independent models were trained and numbers obtained were averaged)
...
# declare -a lamda_arr=(0.1) # meta-learning rate
...
do
   ...
   do
      ...
      do
         for Q in "${model_id[@]}"
         do
            nohup ./my_TREC.sh $mode > $logdir/"$MODE"_"$GAMMA"_"$LAMDA"_"$Q".txt &
         done
      done
   done  
done